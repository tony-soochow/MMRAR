...
def main(rollout,env_id,LOAD,Budget):
    ...
    parser.add_argument('--env_id', type=str, default='Walker')
    ...
    parser.add_argument('--txt', type=str,required=False,default='SW_simple_record_621.txt')
    args = parser.parse_args()
    
    args.env_id=env_id
    global filename
    filename = 'Swimmer-2v/'+args.txt
...
if __name__ == '__main__':
    ...
    root_folder = 'Swimmer-2v/final_test/test_W_T/'
...
# 初始化一个颜色字典，用于存储每个文件夹的颜色
    ...
    for k in range(0,4):
        ...
        for folder in subfolders:
            match = re.match(r'.*/(\d{8}T\d{6}.\d+)WD_(\w+)/2000000_finish', folder)
            match = re.match(r'.*/(\d+T\d+\.\d+)WD_(\w+)/2000000_finish', folder)
            match = re.match(r'.*/\d+T\d+\.\d+WD_(\w+)/2000000_finish', folder)
            ...
            if match:
                ...
                # print("*****"*80)
                    ...
                    average_rewards[variable_name].append(main(rollout, env_id, folder+'/2000000_finish', Budget))
                ...
                filename='Swimmer-2v/final_test/test_W_T/simple.txt'
